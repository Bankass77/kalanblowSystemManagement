...
public class KalanblowApplication {
     ...
     * 
     * boolean emailExist = userService.emailExist("admin@example.com");
     * 
     ...
     * adminDto.setLastName("admin2"); adminDto.setPassword(passwordEncoder.encode("ExaMple2021!"));
     * adminDto.setMobileNumber("0256369649"); adminDto.setAdmin(true).setBirthDate(LocalDate.now())
     ...
     * adminDto.setGender(Gender.FEMALE); adminDto.setCreatedDate(LocalDateTime.now());
     * adminDto.setLastModifiedDate(LocalDateTime.now()); adminDto.setAdmin(true);
     * adminDto.setBirthDate(LocalDate.of(1990, Month.APRIL, 5)); Addresse addresse = new
     * Addresse(); addresse.setCity("Paris"); addresse.setCodePostale(75016);
     * addresse.setCountry("France"); addresse.setState("Ile-De-France");
     * addresse.setStreet("Rue Voltaire"); addresse.setStreetNumber(35);
     * adminDto.setAdresse(addresse); System.out.println("adminDto is:" + adminDto);
